...
int X, Y, velMotor; // As coordenadas em X, Y e a variavel de velocidade
unsigned long time1; // Variavel auxiliar para guardar tempo
boolean bluetoothConectado = false; // Variável para rastrear o estado da conexão Bluetooth
...
void loop() {
  if (Serial.available() > 0) { // Verificando se há dados para receber do bluetooth.
    ...
    if (in_byte == (255)) {  // Verifica se é o byte de encerramento do dado
      array_index = 0;
      bluetoothConectado = true; // Marca a conexão como estabelecida
    }
    ...
    if ((millis() - time1) > 500) { // Compara se passaram mais que 0,5s desde a ultima vez que o bluetooth estava disponivel
      ...
      bluetoothConectado = false; // Marca a conexão como perdida
    }
  }
  if (bluetoothConectado) { // Verifica se o bluetooth está conectado antes de calcular X e Y
    X = dataIn[2] - 124; // Calculando o valor do eixo X do Joystick
    Y = 124 - dataIn[3]; // Calculando o valor do eixo Y do Joystick  
